...
class BookUpdate(UpdateView):
    model = Book
    fields = {'title', 'author', 'genre', 'summary', 'publisher'}
...
def signup(request):
    if request.method == 'POST':
        ...
        form = SignupForm()
    return render(request, 'registration/register.html', {'form': form})                                   #added
...
def activate(request, uidb64, token):
    ...
    if user is not None and account_activation_token.check_token(user, token):
        ...
        return HttpResponse('Activation link is invalid!')                                   #added


def Borrower(request):
    user = request.user
    bookinstance = BookInstance.objects.get(id=request.POST['id'])
    if request.method == 'POST':
        if '_borrow' in request.POST:
...
class SearchResultsView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if query == '':
            ...
            print(words)
            search_query = SearchQuery(words.pop())
            for w in words:
                search_query &= SearchQuery(w)

            search_vector = SearchVector('title')

            object_list = Book.objects.annotate(search=search_vector).filter(search=search_query)
